...
(`procedures.json`, `mapseq_acquisition.json`, `barseq_acquisition.json`)
for the two subjects in the LC paper: **780345** and **780346**.
...
this capsule, so a future reader can reproduce the exact bytes of any
...
│   ├── mapseq_acquisition.json    # MAPseq acquisition (chunk-level specimen IDs)
│   └── barseq_acquisition.json    # BARseq acquisition (section-level specimen IDs)
└── 780346/
    ├── procedures.json
    ├── mapseq_acquisition.json
    └── barseq_acquisition.json
```

`procedures.json` is **identical** between the BARseq and MAPseq data assets
for a given subject — the brain was sectioned once and both modalities used
the same slides. The file therefore covers MAPseq batches (300 µm partial
slices, plates 0–98 and 112–132), BARseq LC sections (20 µm uniform, plates
99–112), the spinal cord, and the per-slide MAPseq chunking. When this
capsule's output is uploaded to S3, the same `procedures.json` will be
attached to both the MAPseq and BARseq data assets.

The two `*_acquisition.json` files are modality-specific and have different
specimen-id semantics:

* **MAPseq** acquisition `specimen_id` is a flat list of the chunk-level
  outputs from the `Sectioning` sub-procedures (e.g. `780345_map001_001`).
  These are the brain-region pieces extracted from each slide and mailed to
  Cold Spring Harbor Laboratory for sequencing.
...
├── run_capsule.py            # iterates SUBJECTS, writes 6 JSONs
├── _procedures_helpers.py    # slide regions + sectioning helpers
├── generators.py             # build_procedures / build_*_acquisition
...
| Tweak generated output (notes, protocol IDs)     | `code/generators.py`          |
